// ignore_for_file: file_names

import 'package:fashion_ecommerce_app/models/base-model.dart';
import 'package:fashion_ecommerce_app/models/categories-model.dart';

List<CategoriesModle> categories = [
  CategoriesModle(
    imageUrl: "assets/image/kids.jpg",
    title: "Kids",
  ),
  CategoriesModle(
    imageUrl: "assets/image/mens.jpg",
    title: "Men",
  ),
  CategoriesModle(
    imageUrl: "assets/image/women.jpg",
    title: "Women",
  ),
  CategoriesModle(
    imageUrl: "assets/image/shoes.jpg",
    title: "Shoes",
  ),
  CategoriesModle(
    imageUrl: "assets/image/bags.jpg",
    title: "Bag",
  ),
  CategoriesModle(
    imageUrl: "assets/image/glass.jpg",
    title: "Glass",
  ),
  CategoriesModle(
    imageUrl: "assets/image/hat.jpg",
    title: "Hat",
  ),
];

final List<BaseModel> mainList = [
  BaseModel(
    imageUrl: "assets/image/mens.jpg",
    name: "Casual Jeans Pant",
    price: 155.99,
    review: 3.6,
    star: 4.8,
    id: 1,
    value: 1,
  ),
  BaseModel(
    imageUrl: "assets/image/blazer.jpg",
    name: "blue Coat",
    price: 143.99,
    review: 5.6,
    star: 5.0,
    id: 2,
    value: 1,
  ),
  BaseModel(
    imageUrl: "assets/image/jacjket.jpg",
    name: "Deep Green Jacket",
    price: 212.99,
    review: 2.6,
    star: 3.7,
    id: 3,
    value: 1,
  ),
  BaseModel(
    imageUrl: "assets/image/shirt.jpg",
    name: "Orange Shirt",
    price: 432.99,
    review: 1.4,
    star: 2.4,
    id: 4,
    value: 1,
  ),
  BaseModel(
    imageUrl: "assets/image/sw.jpg",
    name: "Grey Pullover",
    price: 112.99,
    review: 4.2,
    star: 1.8,
    id: 5,
    value: 1,
  ),
  BaseModel(
    imageUrl: "assets/image/women.jpg",
    name: "Pullover Sleeveless",
    price: 320.99,
    review: 2.1,
    star: 3.1,
    id: 6,
    value: 1,
  ),
  BaseModel(
    imageUrl: "assets/image/womens.jpg",
    name: "Black Coat",
    price: 113.99,
    review: 3.1,
    star: 4.8,
    id: 7,
    value: 1,
  ),
  BaseModel(
    imageUrl: "assets/image/t-shirt.jpg",
    name: "White Shirt",
    price: 178.99,
    review: 2.6,
    star: 4.8,
    id: 8,
    value: 1,
  ),
];

List<BaseModel> itemsOnCart = [];
List<BaseModel> itemsOnSearch = [];
